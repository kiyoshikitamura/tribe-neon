...
try {
  ...
  await build({ entryPoints: [join(root, 'tests/raid-room/ranking-retirement.test.tsx')], outfile: output, bundle: true,
    ...
    plugins: [{ name: 'test-game-context', setup(build) {
      ...
      build.onLoad({filter: /.*/,namespace:'stub'},args=>({contents:args.path==='db'?'export const supabase = { rpc: (...args)=>globalThis.__rankingRpc(...args) };':'export const useScreenReadiness = () => ({status: "ready",retry(){}});',loader:'js'}));
    } }],
  });
