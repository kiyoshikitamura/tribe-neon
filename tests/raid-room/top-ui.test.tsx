...
test('敵のタップは選択callbackだけを発火',async()=>{const h=mount('empty');await ready(h);fireEvent.click(h.view.getAllByRole('button',{name:'挑む'})[0]);assert.deepEqual(h.calls,[['choose',RAID_TOP_ENEMIES[0].variantId]]);});
test('主催/通常参加/救援参加を保持し人数をonline扱いしない',async()=>{const h=mount('multiple');await ready(h);const region=within(h.view.getByRole('region',{name:'参戦中'}));for(const text of ['主催','参加中','救援参加'])assert.ok(region.getByText(text));assert.doesNotMatch(h.view.container.textContent||'',/オンライン|楽勝|討伐困難/);});
test('終了した救援は戦況を見る導線に変更',async()=>{const h=mount('ended');await ready(h);assert.equal(h.view.queryAllByRole('button',{name:'救援に向かう'}).length,0);assert.equal(h.view.getAllByRole('button',{name:'戦況を見る'}).length,6);});
