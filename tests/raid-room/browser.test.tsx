...
test('救援依頼の通信失敗後は同requestで再試行し、両公開先の回数を更新', async () => {
 ...
 try {fireEvent.click(await ui.findByRole('button',{name:'救援を依頼'}));await ui.findByRole('alert');fireEvent.click(ui.getByRole('button',{name:'救援依頼の送信結果を確認'}));await ui.findByText('救援依頼を送信しました。');await waitFor(()=>assert.deepEqual(Array.from(ui.container.querySelectorAll('.raid-rescue-panel__channels dd')).map(el=>el.childNodes[0]?.textContent?.trim()),['1','1']));assert.equal(ids.length,2);assert.equal(ids[0],ids[1]);assert.deepEqual(blocking,[true,false,true,false]);}finally{cleanup();}
});
