...
try {
  ...
  await build({ entryPoints: [join(root, 'tests/raid-room/street-presentation.test.tsx')], outfile: output, bundle: true,
    platform: 'node', format: 'cjs', jsx: 'automatic', packages: 'external', loader: { '.css': 'empty' },
    plugins: [{ name: 'test-game-context', setup(build) {
      build.onResolve({ filter: /(GameContext|AudioProvider)$/ }, () => ({ path: 'game-context', namespace: 'test' }));
      build.onLoad({ filter: /.*/, namespace: 'test' }, () => ({ contents: 'export const useGame = () => ({ playCyberSe() {} }); export const useAudio = () => ({ playSe() {}, playCyberSe() {} });', loader: 'js' }));
    } }],
  });
