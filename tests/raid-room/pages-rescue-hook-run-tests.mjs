...
try {
  ...
  await build({ entryPoints: [join(root, 'tests/raid-room/pages-rescue-hook.test.tsx')], outfile: output, bundle: true,
    platform: 'node', format: 'cjs', jsx: 'automatic', packages: 'external', loader: { '.css': 'empty' },
    plugins: [{ name: 'test-game-context', setup(build) {
      build.onResolve({filter:/utils\/supabase$/},()=>({path:'supabase',namespace:'rpc-test'}));
      build.onLoad({filter:/.*/,namespace:'rpc-test'},()=>({contents:'export const supabase={rpc:(...args)=>globalThis.qaRpc(...args)};',loader:'js'}));
      build.onResolve({ filter: /GameContext$/ }, () => ({ path: 'game-context', namespace: 'test' }));
      build.onLoad({ filter: /.*/, namespace: 'test' }, () => ({ contents: 'export const useGame = () => globalThis.qaContext;', loader: 'js' }));
    } }],
  });
  const setup = join(temp, 'dom.cjs');
  await writeFile(setup, `const {JSDOM}=require('jsdom'); const dom = new JSDOM('<!doctype html><html><body></body></html>', {url:'http://localhost'}); for (const key of ['window','document','HTMLElement','HTMLButtonElement','Node','MutationObserver','getComputedStyle']) globalThis[key] = dom.window[key]; Object.defineProperty(globalThis,'navigator',{value:dom.window.navigator,configurable:true}); globalThis.requestAnimationFrame=(fn)=>setTimeout(fn,0); globalThis.cancelAnimationFrame=clearTimeout; globalThis.Image=class {naturalWidth=100;complete=true;set src(value){this._src=value;queueMicrotask(()=>this.onload?.());}get src(){return this._src;}decode(){return Promise.resolve();}}; globalThis.ResizeObserver=class{observe(){}unobserve(){}disconnect(){}}; globalThis.IS_REACT_ACT_ENVIRONMENT=true;`);
