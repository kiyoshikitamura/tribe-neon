...
test('integration preserves independent Skill and Equipment inventory filters', async ({page}) => {
 await enterGame(page);
 await page.locator('.footer-item[aria-label="キャラ"]').click();
 await page.getByRole('button', {name:'育成する', exact:true}).click();
 const navigation=page.getByRole('navigation', {name:'キャラクター管理'});
 await navigation.getByRole('button', {name:'スキル', exact:true}).click();
 const inventory=page.locator('.character-v2-asset-list');
 await expect(inventory.locator('.character-v2-asset-card')).toHaveCount(2);
 await inventory.getByRole('button', {name:'装備中', exact:true}).click();
 await expect(inventory.locator('.character-v2-asset-card')).toHaveCount(1);
 await navigation.getByRole('button', {name:'装備', exact:true}).click();
 await expect(inventory.locator('.character-v2-asset-card')).toHaveCount(2);
 await inventory.getByRole('button', {name:'装備中', exact:true}).click();
 await expect(inventory.locator('.character-v2-asset-card')).toHaveCount(1);
 await inventory.locator('.character-v2-asset-card').click();
 await expect(page.getByRole('dialog', {name:'装備詳細'})).toBeVisible();
});
