...
for (const viewport of [{ width: 390, height: 844 }, { width: 412, height: 915 }]) {
  test(`VS First View, switch, Ready and cancellation ${viewport.width}`, async ({ page }) => {
    ...
    await expect(hero.locator('.is-player .battle-top-username')).toHaveText('プレイヤー07');
    await expect(hero.locator('.is-player .battle-top-rate')).toContainText('1,500');
    await expect(hero).toContainText('公式戦 ｜ BP 1消費 ｜ RATE変動あり');
    ...
    await expect(hero.locator('.is-rival .battle-top-rate')).toHaveText(await rivals.nth(1).locator('.battle-top-rate').innerText());
    await expect(page.locator(".battle-top-start")).toBeEnabled();
    ...
    await page.getByRole('button', { name: '相手のデッキを見る', exact: true }).click();
    const rivalDeck = page.getByRole('dialog', { name: 'RIVAL DECK', exact: true });
    await expect(rivalDeck.locator('.battle-top-deck-summary')).toContainText('ケンゴ');
    // Accepted Mock returns a fixed three-character public roster; live fixtures verify five.
    await expect(rivalDeck.locator('.pvp-deck-member')).toHaveCount(3);
    expect(await rivalDeck.evaluate(el => el.scrollWidth - el.clientWidth)).toBe(0);
    const dialogBox = (await rivalDeck.boundingBox())!;
    expect(dialogBox.x).toBeGreaterThanOrEqual(0);
    expect(dialogBox.y + dialogBox.height).toBeLessThanOrEqual(viewport.height);
    await page.screenshot({ path: `scratch/battle-top/polish-rival-deck-${viewport.width}.png` });
    await rivalDeck.getByRole('button', { name: '閉じる', exact: true }).last().click();
    await page.getByRole('button', { name: '自分のデッキを見る', exact: true }).evaluate((button: HTMLButtonElement) => {button.click(); button.click();});
    const myDeck = page.getByRole('dialog', { name: 'MY DECK', exact: true });
    await expect(myDeck).toHaveCount(1);
    await expect(myDeck.locator('.battle-top-deck-summary')).toContainText('プレイヤー07');
    await expect(myDeck.locator('.pvp-deck-member')).toHaveCount(5);
    await expect(myDeck.locator('.shared-skill-icon')).toHaveCount(5);
    await page.screenshot({ path: `scratch/battle-top/polish-my-deck-${viewport.width}.png` });
    await myDeck.getByRole('button', { name: '閉じる', exact: true }).last().click();
    await expect(page.getByText('出撃編成・スキルを確認', { exact: true })).toHaveCount(0);
    await expect(page.getByText('選択中の相手・出撃編成を確認', { exact: true })).toHaveCount(0);
...
test("ranking, Raid, reload and foreground return", async ({ page }) => {
  ...
  await page.locator(".circle-menu-btn.fight").click();
  await page.getByRole("button", { name: "レイドを見る", exact: true }).click();
