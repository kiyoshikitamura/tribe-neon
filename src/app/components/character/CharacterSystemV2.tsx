...
import { canonicalItemEffectValue, canonicalItemName } from "@/domain/gameplay/canonical/items";
...
import { calculateGrowthExp } from "@/domain/gameplay/canonical/growthExp";
import { useGrowthExpMaster } from "@/app/context/hooks/useGrowthExpMaster";
import GrowthExpSummary from "./GrowthExpSummary";
...
export default function CharacterSystemV2({ initialCharacterMasterId, setupResult }: { initialCharacterMasterId?: string; setupResult?: any }) {
  ...
  const growthMaster = useGrowthExpMaster(game.session?.user?.id);
  ...
  const gainedCharacterExp = Object.entries(growthCounts).reduce((sum, [id, count]) => sum + count * (canonicalItemEffectValue(id) ?? 0), 0);
  const characterPreview = growthMaster.master && selectedCharacter
    ? calculateGrowthExp(Number(selectedCharacter.level || 1), Number(selectedCharacter.xp || 0), characterLevelCap, gainedCharacterExp, growthMaster.master.character) : null;
  const characterAfterLevel = characterPreview?.level ?? Number(selectedCharacter?.level || 1);
  const characterAfterStats = getCharacterTotalStats({ ...selectedCharacter, level: characterAfterLevel }, game.userEquipmentsList || []);
  const growthRecord = assetGrowth?.kind === "skill" ? (game.selectedSkill || assetGrowth.record) : assetGrowth?.kind === "equipment" ? (game.selectedEquipment || assetGrowth.record) : game.selectedEquipment;
  const equipmentMaterialCount = Object.values(equipmentGrowthCounts).reduce((sum, count) => sum + Number(count || 0), 0);
  const equipmentLb = Number(game.selectedEquipment?.plus_val || 0);
  const equipmentLbRequired = CANONICAL_EQUIPMENT_LIMIT_BREAK.cost_curve[equipmentLb] ?? 0;
  const equipmentLbMaxed = equipmentLb >= CANONICAL_EQUIPMENT_LIMIT_BREAK.max_level;
  const skillLbMaxed = Number(game.selectedSkill?.plus_val || 0) >= 10;
  const equipmentCurrentLevel = Number(growthRecord?.level || 1);
  const gainedEquipmentExp = Object.entries(equipmentGrowthCounts).reduce((sum, [id, count]) => sum + count * (canonicalItemEffectValue(id) ?? 0), 0);
  const equipmentPreview = growthMaster.master && growthRecord && assetGrowth?.kind !== "skill"
    ? calculateGrowthExp(equipmentCurrentLevel, Number(growthRecord.xp || 0), getEquipmentLevelCap(Number(growthRecord.plus_val || 0)), gainedEquipmentExp, growthMaster.master.equipment) : null;
  const equipmentAfterLevel = equipmentPreview?.level ?? equipmentCurrentLevel;
  const characterGrowthDisabled = !characterPreview?.canApply || characterPreview.cashSpent > Number(game.cash || 0);
  const equipmentGrowthDisabled = !equipmentPreview?.canApply || equipmentPreview.cashSpent > Number(game.cash || 0);
  ...
  const runCharacterGrowth = async () => {
    if (characterGrowthDisabled) return;
  ...
  const runEquipmentGrowth = async () => {
    if (equipmentGrowthDisabled) return;
    ...
    return <div className="character-v2-asset-grid" aria-label={kind === "skill" ? "所持スキル" : "所持装備"}>
      {records.map((record: any) => {
        ...
        return <button type="button" key={record.id} className="character-v2-asset-card active-scale-effect" onClick={() => setAssetDetail({ kind, record, master })}>
          ...
          <span className="character-v2-card-level">{kind === "equipment" && <>Lv.{Number(record.level || 1)} / </>}+{Number(record.plus_val || 0)}</span>
  ...
  return <div className="character-v2-shell">
    ...
    {mainView === "CHARACTERS" && characterView === "GROWTH" && <fieldset className="character-v2-view character-v2-growth character-v2-pending-surface" disabled={game.upgradeLoading} aria-busy={game.upgradeLoading}>
      ...
      {growthTab === "LEVEL" && <section className="character-v2-operation-card"><h3>レベル</h3><div className="character-v2-current-after"><span><small>現在</small><strong>Lv.{Number(selectedCharacter.level || 1)}</strong></span><i>→</i><span className={characterMaterialCount > 0 ? "has-preview" : ""}><small>強化後</small><strong>Lv.{characterAfterLevel}</strong></span></div>
        {characterPreview && characterPreview.levelsGained > 0 && <dl className="character-v2-preview-stats">{(["hp", "atk", "def", "spd", "luk"] as const).map((key) => <div key={key}><dt>{key.toUpperCase()}</dt><dd>{Number(stats[key]).toLocaleString()} → <b>{Number(characterAfterStats[key]).toLocaleString()}</b></dd></div>)}</dl>}
        {(["CHAR_EXP_S", "CHAR_EXP_M", "CHAR_EXP_L"] as const).map((itemId) => { const owned = Number(game[itemId === "CHAR_EXP_S" ? "charExpS" : itemId === "CHAR_EXP_M" ? "charExpM" : "charExpL"] || 0); return <div className="character-v2-material" key={itemId}><CanonicalItemIcon itemId={itemId} alt="" /><div><strong>{canonicalItemName(itemId)}</strong><span>所持 {owned} / 使用 {growthCounts[itemId]}</span></div><div className="character-v2-counter"><button onClick={() => updateCount(setGrowthCounts, itemId, -1, owned)}>−</button><b>{growthCounts[itemId]}</b><button disabled={Number(selectedCharacter.level || 1) >= 100} onClick={() => updateCount(setGrowthCounts, itemId, 1, owned)}>＋</button></div></div>; })}
        <GrowthExpSummary preview={characterPreview} error={growthMaster.error} retry={growthMaster.retry} /><OutlawButton variant="primary" fullWidth isLoading={game.upgradeLoading} disabled={characterGrowthDisabled} onClick={() => void runCharacterGrowth()}>{characterMaterialCount === 0 && characterPreview?.levelsGained ? "保持EXPで強化する" : "強化する"}</OutlawButton>
      </section>}
      ...
      {growthTab === "SKILL" && <section className="character-v2-loadout-summary"><header><strong>装備中スキル</strong><span>{skillSlots}枠</span></header><OutlawButton variant="primary" fullWidth isLoading={game.upgradeLoading} disabled={game.upgradeLoading} onClick={() => void game.handleEquipSkillBulkRecommended(selectedCharacter.id, selectedMaster.id)}>おすすめスキル</OutlawButton><div className="character-v2-summary-grid">{Array.from({ length: canonicalSkillSlotCount(CHARACTER_AWAKENING_MAX_LEVEL) }).map((_, index) => { const unlocked = index < skillSlots; const record = equippedSkills.find((entry: any) => Number(entry.slot_index) === index); const master = record && CANONICAL_SKILL_VIEW.find((entry: any) => entry.id === (record.skill_card_id || record.skill_id)); return <button key={index} disabled={!unlocked} aria-label={"スキル枠" + (index + 1)} className={selectedSkillSlot === index ? "active" : ""} onClick={() => { setSelectedSkillSlot(index); if (master) setAssetDetail({kind:"skill",record,master}); }}>{!unlocked ? <span>未解放</span> : master ? <><SkillArt master={master} /><small>{master.name}</small><small>+{Number(record.plus_val || 0)}</small></> : <span>＋ スキルを装備</span>}</button>; })}</div>{selectedSkillSlot !== null && <><h3>付け替えるスキル</h3>{renderAssetGrid("skill")}</>}</section>}
    </fieldset>}
    ...
    {(mainView === "SKILLS" || mainView === "EQUIPMENT") && assetGrowth && <fieldset className="character-v2-view character-v2-asset-growth character-v2-pending-surface" disabled={game.upgradeLoading} aria-busy={game.upgradeLoading}><header className="character-v2-title"><button onClick={() => { setMainView("CHARACTERS"); setCharacterView(assetGrowth.kind === "skill" ? "GROWTH" : "LOADOUT"); setAssetGrowth(null); }}>戻る</button><strong>{assetGrowth.kind === "skill" ? "スキル強化" : "装備強化"}</strong><span>{selectedMaster.jpName}</span></header><div className="character-v2-growth-asset">{assetGrowth.kind === "skill" ? <SkillArt master={assetGrowth.master} /> : <EquipmentArt master={assetGrowth.master} />}<div><strong>{assetGrowth.master.name}</strong><RarityBadge rarity={assetGrowth.master.rarity} /></div></div>
      {assetGrowth.kind === "skill" ? <section className="character-v2-operation-card"><h3>限界突破</h3><div className="character-v2-current-after"><span><small>現在</small><strong>+{Number(growthRecord?.plus_val || 0)}</strong></span><i>→</i><span className="has-preview"><small>強化後</small><strong>+{Math.min(10, Number(growthRecord?.plus_val || 0) + 1)}</strong></span></div><div className="character-v2-material"><CanonicalItemIcon itemId="SKILL_MANUAL" alt="" /><div><strong>{canonicalItemName("SKILL_MANUAL")}</strong><span>所持 {Number(game.skillManuals || 0)} / 必要 1</span></div></div><div className="character-v2-consumption"><span>必要CASH {(Number(growthRecord?.plus_val || 0) + 1) * 1000}</span><span>同名カード または 代用素材</span></div><div className="character-v2-two-actions"><OutlawButton disabled={game.upgradeLoading || skillLbMaxed} onClick={() => void game.handleSkillUpgrade(false)}>同名カード</OutlawButton><OutlawButton variant="primary" disabled={game.upgradeLoading || skillLbMaxed || Number(game.skillManuals || 0) < 1} onClick={() => void game.handleSkillUpgrade(true)}>限界突破する</OutlawButton></div></section>
      : <section className="character-v2-operation-card"><h3>Lv強化</h3><div className="character-v2-current-after"><span><small>現在</small><strong>Lv.{equipmentCurrentLevel}</strong></span><i>→</i><span className={equipmentMaterialCount > 0 ? "has-preview" : ""}><small>強化後</small><strong>Lv.{equipmentAfterLevel}</strong></span></div>{(["EQUIP_EXP_S", "EQUIP_EXP_M", "EQUIP_EXP_L"] as const).map((itemId) => { const owned = Number(game[itemId === "EQUIP_EXP_S" ? "equipExpS" : itemId === "EQUIP_EXP_M" ? "equipExpM" : "equipExpL"] || 0); return <div className="character-v2-material" key={itemId}><CanonicalItemIcon itemId={itemId} alt="" /><div><strong>{canonicalItemName(itemId)}</strong><span>所持 {owned} / 使用 {equipmentGrowthCounts[itemId]}</span></div><div className="character-v2-counter"><button onClick={() => updateCount(setEquipmentGrowthCounts, itemId, -1, owned)}>−</button><b>{equipmentGrowthCounts[itemId]}</b><button disabled={equipmentCurrentLevel >= 100} onClick={() => updateCount(setEquipmentGrowthCounts, itemId, 1, owned)}>＋</button></div></div>; })}<GrowthExpSummary preview={equipmentPreview} error={growthMaster.error} retry={growthMaster.retry} /><OutlawButton variant="primary" fullWidth isLoading={game.upgradeLoading} disabled={equipmentGrowthDisabled} onClick={() => void runEquipmentGrowth()}>{equipmentMaterialCount === 0 && equipmentPreview?.levelsGained ? "保持EXPで強化する" : "強化する"}</OutlawButton><h3>限界突破</h3><div className="character-v2-current-after"><span><small>現在</small><strong>+{Number(growthRecord?.plus_val || 0)}</strong></span><i>→</i><span className="has-preview"><small>強化後</small><strong>+{Math.min(10, Number(growthRecord?.plus_val || 0) + 1)}</strong></span></div><div className="character-v2-material"><CanonicalItemIcon itemId="EQUIP_LB_PART" alt="" /><div><strong>{canonicalItemName("EQUIP_LB_PART")}</strong><span>所持 {Number(game.equipLbParts || 0)} / 必要 {equipmentLbRequired}</span></div></div><div className="character-v2-consumption"><span>CASH不要</span><span>同名アイテム1個 または 改造パーツ{equipmentLbRequired}個</span></div><div className="character-v2-two-actions"><OutlawButton disabled={game.upgradeLoading || equipmentLbMaxed} onClick={() => void game.handleEquipmentLimitBreak(false)}>同名アイテム</OutlawButton><OutlawButton variant="primary" disabled={game.upgradeLoading || equipmentLbMaxed || Number(game.equipLbParts || 0) < equipmentLbRequired} onClick={() => void game.handleEquipmentLimitBreak(true)}>限界突破する</OutlawButton></div></section>}
    </fieldset>}
    ...
    {(mainView === "SKILLS" || mainView === "EQUIPMENT") && !assetGrowth && <fieldset className="character-v2-view character-v2-asset-list character-v2-pending-surface" disabled={game.upgradeLoading} aria-busy={game.upgradeLoading}>
      ...
      {mainView === "SKILLS" && game.selectedSkill && <section className="character-v2-operation-card"><h3>限界突破</h3><div className="character-v2-current-after"><strong>+{Number(game.selectedSkill.plus_val || 0)}</strong><i>→</i><strong>+{Math.min(10, Number(game.selectedSkill.plus_val || 0) + 1)}</strong></div><div className="character-v2-material"><CanonicalItemIcon itemId="SKILL_MANUAL" alt="" /><div><strong>{canonicalItemName("SKILL_MANUAL")}</strong><span>所持 {Number(game.skillManuals || 0)} / 必要 1</span></div></div><div className="character-v2-two-actions"><OutlawButton disabled={game.upgradeLoading || skillLbMaxed} onClick={() => void game.handleSkillUpgrade(false)}>同名カード</OutlawButton><OutlawButton variant="primary" disabled={game.upgradeLoading || skillLbMaxed || Number(game.skillManuals || 0) < 1} onClick={() => void game.handleSkillUpgrade(true)}>限界突破する</OutlawButton></div></section>}
      {mainView === "EQUIPMENT" && game.selectedEquipment && <section className="character-v2-operation-card"><h3>装備強化</h3><div className="character-v2-current-after"><strong>Lv.{Number(game.selectedEquipment.level || 1)}</strong><i>→</i><strong>Lv.{equipmentAfterLevel}</strong></div>{(["EQUIP_EXP_S", "EQUIP_EXP_M", "EQUIP_EXP_L"] as const).map((itemId) => { const owned = Number(game[itemId === "EQUIP_EXP_S" ? "equipExpS" : itemId === "EQUIP_EXP_M" ? "equipExpM" : "equipExpL"] || 0); return <div className="character-v2-material" key={itemId}><CanonicalItemIcon itemId={itemId} alt="" /><div><strong>{canonicalItemName(itemId)}</strong><span>所持 {owned}</span></div><div className="character-v2-counter"><button onClick={() => updateCount(setEquipmentGrowthCounts, itemId, -1, owned)}>−</button><b>{equipmentGrowthCounts[itemId]}</b><button disabled={equipmentCurrentLevel >= 100} onClick={() => updateCount(setEquipmentGrowthCounts, itemId, 1, owned)}>＋</button></div></div>; })}<GrowthExpSummary preview={equipmentPreview} error={growthMaster.error} retry={growthMaster.retry} /><OutlawButton variant="primary" fullWidth disabled={game.upgradeLoading || equipmentGrowthDisabled} onClick={() => void runEquipmentGrowth()}>{equipmentMaterialCount === 0 && equipmentPreview?.levelsGained ? "保持EXPで強化する" : "強化する"}</OutlawButton><h3>限界突破</h3><div className="character-v2-current-after"><strong>+{Number(game.selectedEquipment.plus_val || 0)}</strong><i>→</i><strong>+{Math.min(10, Number(game.selectedEquipment.plus_val || 0) + 1)}</strong></div><div className="character-v2-material"><CanonicalItemIcon itemId="EQUIP_LB_PART" alt="" /><div><strong>{canonicalItemName("EQUIP_LB_PART")}</strong><span>所持 {Number(game.equipLbParts || 0)} / 必要 {equipmentLbRequired}</span></div></div><div className="character-v2-two-actions"><OutlawButton disabled={game.upgradeLoading || equipmentLbMaxed} onClick={() => void game.handleEquipmentLimitBreak(false)}>同名装備</OutlawButton><OutlawButton variant="primary" disabled={game.upgradeLoading || equipmentLbMaxed || Number(game.equipLbParts || 0) < equipmentLbRequired} onClick={() => void game.handleEquipmentLimitBreak(true)}>限界突破する</OutlawButton></div></section>}
    </fieldset>}
