...
import CharacterStatusBadges, { RarityBadge } from "./CharacterStatusBadges";
...
export default function CharacterParty({game,characters,onBack}: {game:any;characters:any[];onBack:()=>void}) {
  ...
  const [view,setView]=useState<"HOME"|"MEMBERS"|"LEADER"|"FRONT">("HOME");
  ...
  return <section className="character-v2-view character-v2-party" aria-busy={pending}>
    <header className="character-v2-title"><button disabled={pending} onClick={view==="HOME"?onBack:()=>{setView("HOME");setDraft([]);setSlot(null);}}> {view==="HOME"?"キャラホームへ戻る":"取消"}</button><strong>{view==="HOME"?"パーティ":view==="MEMBERS"?"メンバー変更":view==="FRONT"?"先頭変更":"リーダー変更"}</strong></header>
    ...
    </>:view==="LEADER"?<><p>リーダーにするキャラクターを選択</p><div className="character-party-leader-candidates">{characters.map((character,i)=><React.Fragment key={character.id}>{card(character.character_id,i,()=>void perform(()=>game.handleSetProfileLeader(character.character_id)))}</React.Fragment>)}</div></>:view==="FRONT"?<><p>先頭にするキャラクターを選択</p><div className="character-party-leader-candidates">{saved.map((id,i)=><React.Fragment key={id}>{card(id,i,()=>void perform(()=>game.handleSaveParty([id,...saved.filter(member=>member!==id)])))}</React.Fragment>)}</div></>:<>
      <p>変更する枠を選択</p><div className="character-party-draft-slots">{draft.map((id,i)=><div key={i} className={slot===i?"active":""}><small>{i===0?"先頭":"枠"+(i+1)}</small>{card(id,i,()=>setSlot(i))}</div>)}</div>
