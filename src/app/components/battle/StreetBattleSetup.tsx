...
export default function StreetBattleSetup({playerParty,enemyParty,playerPower,enemyPower,tutorial,mode,label,background,tactic,onTactic,onStart,onBack,startLabel,backLabel,resourceLabel,enemyLeader,enemyDetails}:{playerParty:Member[];enemyParty:Member[];playerPower:number;enemyPower:number;tutorial:boolean;mode:string;label:string;background?:string;tactic:string;onTactic:(value:string)=>void;onStart:()=>void;onBack?:()=>void;startLabel?:string;backLabel?:string;resourceLabel?:string;enemyLeader?:Pick<Member,"characterId"|"name">;enemyDetails?:ReactNode}) {
  ...
  const sources = JSON.stringify([...playerParty,...enemyParty,...(enemyLeader?[enemyLeader]:[])].flatMap(m=>{const c=master(m);return c?[getCharacterTransparentImg(c.name),getRarityBadgeAsset(c.rarity),getAttributeBadgeAsset(c.alignment)!]:[]}).concat([background || "/bg/bg_street_shinjuku.jpg","/effects/battle-v3/street-impact.webp","/effects/battle-v3/street-support.webp"]));
