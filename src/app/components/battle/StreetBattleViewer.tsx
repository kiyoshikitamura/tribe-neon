...
import { battleStatusApplyLabel, battleStatusPresentationTone } from "@/domain/presentation/battleStatusPresentation";
...
function Resolution({group,special,impactBeat,multiple}:{group:BattleTargetResolutionGroup;special:boolean;impactBeat:boolean;multiple:boolean}) {
  ...
  const labels = [...new Set(effects.map(e=>battleStatusApplyLabel(e.payload)))];
  const shieldDamage = damageEvents.reduce((sum,e)=>sum+Math.max(0,Number(e.payload.shieldDamage ?? 0)),0);
  return <div className={`sb-resolution ${positive?"positive":"negative"} ${multiple?"multiple":""}`}>
    ...
    <div className="sb-numbers">{damageEvents.some(e=>e.payload.critical===true)&&<small>CRITICAL</small>}{damageEvents.length>0&&<b data-battle-number="damage">{damageEvents.every(e=>e.payload.hit===false)?"MISS":`−${damage.toLocaleString()}`}</b>}{healEvents.length>0&&<b data-battle-number="heal" className="positive">+{healing.toLocaleString()}</b>}{shieldDamage>0&&<small>吸収 {shieldDamage.toLocaleString()}</small>}{labels.map(label=><small key={label}>{label}</small>)}</div>
  </div>;
}
...
export default function StreetBattleViewer(props:QuestBattleViewerProps) {
  ...
  return <div className="playing-container sb-root" data-battle-speed={props.speed} data-action-phase={(props.presentationPhase || "IDLE").toLowerCase().replaceAll("_","-")} data-action-actor-id={actor?.id} data-skill-rarity={skillRarity} data-acceptance-state={props.tutorial ? isSkill?"B4":"B3" : undefined} style={{"--sb-background":`url('${props.backgroundPath || "/bg/bg_street_shinjuku.jpg"}')`} as CSSProperties}>
    <header className="sb-header"><div><small>TRIBE NEON</small><h1>{props.opponentName || "BATTLE"}</h1></div><span data-displayed-round={props.round} data-configured-round-limit={roundLimit}>ROUND <b>{String(props.round).padStart(2,"0")}</b><small> / {roundLimit}</small></span></header>
    ...
    <p className="sb-event" aria-live="polite">{action ? `${actor?.name || ""} / ${skillName}` : props.paused ? "一時停止中" : "AUTO BATTLE"}</p>
    ...
    <footer className="sb-controls"><button onClick={()=>{props.onSpeedChange(props.speed===2?1:props.speed===1&&props.monthlyPassActive?3:2);props.onSound();}}>×{props.speed}</button><button onClick={()=>{props.onPauseChange(!props.paused);props.onSound();}}>{props.paused?"再開":"一時停止"}</button>{props.canSkip&&<button disabled={props.skipPending} onClick={props.onSkip}>{props.skipPending?"結果へ移動中":"SKIP"}</button>}{!props.tutorial&&<button onClick={props.onRetreat}>撤退</button>}</footer>
  </div>;
}
