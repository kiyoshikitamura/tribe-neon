...
export default function StreetBattleViewer(props:QuestBattleViewerProps) {
  ...
  const actorId = action?.unit.actorId ?? (props.skillCutIn ? props.skillCutIn.actorId : props.targetLine?.fromId ?? props.authoritativeTimeline?.[0]?.id ?? props.timeline[props.timelineIndex]?.id);
  ...
  const roundLimit = props.roundLimit ?? (props.battleMode==="RAID" ? 30 : props.battleMode==="PATROL" ? 15 : 20);
  return <div className="playing-container sb-root" data-battle-speed={props.speed} data-action-phase={(props.presentationPhase || "IDLE").toLowerCase().replaceAll("_","-")} data-action-actor-id={actor?.id} data-skill-rarity={skillRarity} data-acceptance-state={props.tutorial ? isSkill?"B4":"B3" : undefined} style={{"--sb-background":`url('${props.backgroundPath || "/bg/bg_street_shinjuku.jpg"}')`} as CSSProperties}>
    ...
    <main className="sb-rosters">{([false,true] as const).map(enemy=><section key={String(enemy)} aria-label={enemy?"敵パーティ":"味方パーティ"}><h2>{enemy?"ENEMY":"YOUR TEAM"}</h2>{(enemy?props.enemyParty:props.playerParty).map(p=>{
      ...
      return <BattleUnitPortrait key={p.id} street participant={p} domId={p.id} imageSrc={c?getCharacterTransparentImg(c.name):undefined} side={enemy?"enemy":"player"} rarity={c?.rarity || p.rarity} attribute={p.alignment || c?.alignment} actor={Boolean(actor?.id===p.id && (action || props.targetLine || props.skillCutIn))} reaction={group} impactOverlay={group ? <Resolution key={action?.unit.replayStartCursor ?? props.skillCutIn?.actionKey} group={group} special={special} impactBeat={action?.beat==="IMPACT"} multiple={groups.length>1}/> : popup ? <div className="sb-resolution">{popup.type==="dmg" && <img className="sb-effect" src={impact} alt=""/>}<div className="sb-numbers"><b data-battle-number={popup.type==="dmg"?"damage":undefined} className={popup.type!=="dmg"?"positive":""}>{popup.type==="dmg"?"−":"+"}{popup.val.toLocaleString()}</b></div></div> : null}/>;
    })}</section>)}</main>
