    ...
    <div className={`setup-container scroll-container ${presentationState === "NAME_INPUT" ? "is-registration" : "is-world-entry"}`} onClick={handleFirstUserInteraction} data-entry-state={presentationState}>
      ...
      {presentationState === "WORLD_INFORMATION" ? (
        <section className={`setup-world-presentation is-stage-${worldStage + 1}`} aria-label="TRIBE NEON プロローグ" data-world-stage={worldStage + 1}>
          ...
          <div className="setup-world-copy" key={worldStage}><TypewriterText text={WORLD_STAGES[worldStage].text} speedMs={46} highlightTerms={[...WORLD_STAGES[worldStage].highlights]} onComplete={() => setWorldStageComplete(true)} /></div>
          <div className="setup-world-progress" aria-hidden="true">{WORLD_STAGES.map((_, index) => <i key={index} className={index <= worldStage ? "is-active" : ""} />)}</div>
          {worldStage === WORLD_STAGES.length - 1 && worldStageComplete && <button className="setup-world-tap" onClick={() => advancePresentation("WORLD_TO_AGEHA")}>TAP TO CONTINUE <span aria-hidden="true">⌄</span></button>}
        </section>
        ...
        <section className="setup-ageha-presentation">
          ...
          <div className="setup-ageha-dialogue"><div className="setup-ageha-name">アゲハ</div><TypewriterText text={AGEHA_INTRO_COPY} speedMs={38} /></div>
