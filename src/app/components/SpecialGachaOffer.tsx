...
import { exchangeItems, parseSpecialGachaCatalog, specialTicketId, type SpecialGacha, type SpecialGachaCatalog, type SpecialGachaId, type SpecialGachaItem } from "@/domain/gacha/specialGacha";
...
export default function SpecialGachaOffer({ category, diamonds, userItems, pending, onScout, onExchange }: {
  ...
  onExchange: (type: SpecialGachaItem["item_type"], id: string, requestId: string) => Promise<boolean>;
}) {
  ...
  useEffect(() => {
    if (pending) return;
    let active = true;
    void Promise.resolve(supabase.rpc("get_special_gacha_catalog")).then(({ data, error }) => {
      if (!active) return;
      const parsed = error ? null : parseSpecialGachaCatalog(data);
  ...
  const exchange = async () => {
    if (!reward) return;
    const key = `${reward.item_type}:${reward.item_id}`;
    if (exchangeRequest.current?.key !== key) exchangeRequest.current = { key, id: crypto.randomUUID() };
    const success = await onExchange(reward.item_type, reward.item_id, exchangeRequest.current.id);
    if (success) {
      exchangeRequest.current = null;
      setReceived(reward.name); setReward(null); setExchangeOpen(false); setRevision(n => n + 1);
    }
  };
  return <section className="special-gacha-offer" aria-label="スペシャルガチャ">
    {offers.map(g => <div key={g.id} className="special-gacha-product">
      <button className="semantic-cta semantic-cta--primary" disabled={pending} onClick={() => { setSelected(g); setChoice(null); }}>{g.name}を引く</button>
      <button className="gacha-rate-link" onClick={() => setRates(g)}>提供割合</button>
    </div>)}
    <button className="special-gacha-pity" disabled={pending} onClick={() => setExchangeOpen(true)}>SSR交換 <span>{catalog.pity_points} / {catalog.pity_cost}Pt</span></button>
    {selected && <CanonicalDialog title={selected.name} onClose={pending ? undefined : () => { setSelected(null); setChoice(null); }} actions={choice ? [
      ...
      {choice ? <p className="special-gacha-confirm">{choice.currency === "DIAMOND" ? `${(selected.cost_diamond * choice.count).toLocaleString()}ダイア` : `チケット${choice.count}枚`}を消費します。</p> : <>
        <div className="gacha-payment-group" aria-label="ダイアで引く">{([1,10] as const).map(count => <button key={count} disabled={pending || !affordable(selected, { count, currency: "DIAMOND" })} onClick={() => setChoice({ count, currency: "DIAMOND" })}><span>{count}連</span><small>{(selected.cost_diamond * count).toLocaleString()}ダイア</small></button>)}</div>
        ...
        <p className="special-gacha-balance">所持 {(diamonds || 0).toLocaleString()}ダイア / チケット{ticketCount(selected)}枚</p>
      </>}
    </CanonicalDialog>}
    {rates && <CanonicalDialog title={`${rates.name} 提供割合`} onClose={() => setRates(null)} actions={[{ label: "閉じる", onClick: () => setRates(null) }]}>
    ...
    {exchangeOpen && <CanonicalDialog title="SSR交換" onClose={pending ? undefined : () => { setExchangeOpen(false); setReward(null); }} actions={reward ? [{ label: "100Ptで交換", semantic: "primary", disabled: pending || catalog.pity_points < catalog.pity_cost, onClick: exchange }] : []}>
      <p>{catalog.pity_points}Pt / 交換に必要：{catalog.pity_cost}Pt</p>
      <div className="special-gacha-exchange">{exchangeItems(catalog.gachas).map(item => <button key={`${item.item_type}:${item.item_id}`} aria-pressed={reward?.item_id === item.item_id && reward.item_type === item.item_type} disabled={pending} onClick={() => setReward(item)}>{item.name}{item.is_exclusive ? "（専用）" : ""}</button>)}</div>
    </CanonicalDialog>}
