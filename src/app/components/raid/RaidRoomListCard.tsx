...
export default function RaidRoomListCard({room,now,enemy,ownerGuild,membership,onOpen,busy}:RaidRoomListCardProps){
 ...
 <div className="raid-list-card__body"><div className="raid-list-card__owner"><span className="raid-list-card__avatar"><RaidPagePortrait src={assets.resolve(icon??RAID_PERSON_FALLBACK)}/></span><div><span>挑戦者</span><strong>{owner?.name??'挑戦者未確認'}</strong><span>{ownerGuild?.status==='available'?ownerGuild.value?.name??'Guild未所属':'Guild未確認'}</span></div>{membership?.status==='available'&&membership.value!=='not_joined'&&<span className="raid-list-card__role">あなた：{({owner:'挑戦者',member:'参加中',rescue:'救援参加',not_joined:''})[membership.value]}</span>}</div>
