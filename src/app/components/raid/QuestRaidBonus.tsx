...
type Bonus={roomId:string;rewardMultiplier?:number;items:{itemId:string;quantity:number}[];issued:boolean;delivery?:'DIRECT'|'PRESENT'};
export default function QuestRaidBonus({roomId}:{roomId:string}){
 ...
 return <details className="raid-reward-items"><summary>報酬ボーナス</summary><p>{bonus.issued?(bonus.delivery==='DIRECT'?'獲得しました':'以前の報酬はプレゼントBOXで確認できます'):'討伐・貢献条件達成で獲得'}</p><ul>{bonus.items.map(item=><li key={item.itemId}>{ITEMS_MASTER_DATA.find(i=>i.id===item.itemId)?.name||'報酬'} × {item.quantity}</li>)}</ul></details>;
}
