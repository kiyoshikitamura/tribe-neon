...
export default function QuestRaidBonus({roomId}:{roomId:string}){
 ...
 return <details className="raid-reward-items"><summary>クエスト発見・撃破ボーナス</summary><p>{bonus.legacyIssued ? '以前の条件で受取済みです' : bonus.issued ? '獲得しました' : '撃破・貢献条件達成で獲得'}</p>{!bonus.legacyIssued && <p>CASH +{bonus.cash.toLocaleString()} / プレイヤーEXP +{bonus.userXp.toLocaleString()}</p>}</details>;
}
