...
export interface RaidEnemySelectionProps {choices:RaidRoomResource<readonly RaidBossChoice[]>;selectedVariantId:string;difficultyId:RaidDifficultyId;onSelectVariant:(id:string)=>void;onSelectDifficulty:(id:RaidDifficultyId)=>void;onConfirm:()=>void;onCancel:()=>void;onRetry:()=>void;busy:boolean;canConfirm?:boolean;error?:string|null;rewardPlan?:RaidRoomResource<RaidRewardPlan>;resolveRewardName?:(id:string)=>string|null|undefined;onEnemyInfo?:(variantId:string)=>void;skillsByCharacterId?:RaidObserved<Readonly<Record<string,readonly {id:string;name:string}[]>>>;}
export default function RaidEnemySelection({choices,selectedVariantId,difficultyId,onSelectVariant,onSelectDifficulty,onConfirm,onCancel,onRetry,busy,canConfirm=true,error,rewardPlan,resolveRewardName,onEnemyInfo,skillsByCharacterId}:RaidEnemySelectionProps){
 const candidates=choices.status==='success'?choices.data??[]:[];
 const entries=candidates.map(choice=>({choice,enemy:resolveRaidTopEnemy(choice.raidVariantId)}));
 const selectedEnemy=entries.find(({choice})=>choice.raidVariantId===selectedVariantId)?.enemy;
 ...
 const assets=useRaidPageAssets([...rewardAssets,...entries.flatMap(({enemy})=>enemy?[{src:enemy.backgroundUrl,fallbackSrc:RAID_BACKGROUND_FALLBACK},{src:enemy.leaderImageUrl,fallbackSrc:RAID_PERSON_FALLBACK}]:[])]);
