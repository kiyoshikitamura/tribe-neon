...
export default function RaidRoomRescueRewardPanel({ client, roomId, onOpenPresents, onDirectReward, plan }: {
  ...
  return <div aria-label="救援報酬">
    ...
    {reward && <>
      {progress && reward.rescueGate.minimumBattles !== null && reward.rescueGate.minimumContributionDamage !== null && <div className="raid-reward-progress" aria-label="救援成功条件">
        <p><strong>{progress.viaRescue ? '✓' : '—'} 救援参加</strong><span>{progress.viaRescue ? '参加済み' : '救援参加が必要'}</span></p>
        <p><strong>{progress.finalizedBattles >= reward.rescueGate.minimumBattles ? '✓' : '—'} 参戦回数</strong><span>{progress.finalizedBattles.toLocaleString('ja-JP')} / {reward.rescueGate.minimumBattles.toLocaleString('ja-JP')}戦</span></p>
        <p><strong>{progress.contributionDamage >= reward.rescueGate.minimumContributionDamage ? '✓' : '—'} 貢献ダメージ</strong><span>{progress.contributionDamage.toLocaleString('ja-JP')} / {reward.rescueGate.minimumContributionDamage.toLocaleString('ja-JP')}</span></p>
