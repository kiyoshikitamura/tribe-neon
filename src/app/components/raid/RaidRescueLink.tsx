...
export default function RaidRescueLink({ rescueId, onOpen, entry, source, status }: RaidRescueLinkProps) {
  ...
  return <section className="raid-rescue-link" aria-label="レイド救援" data-rescue-id={rescueId} data-room-id={card?.room.roomId}>
    ...
    {card ? <>
      ...
      <div className="raid-rescue-link__battle"><span>{state === 'cleared' ? '撃破成功' : expired ? '時間切れ' : minutes === null ? '残り時間未取得' : `残り${Math.floor(minutes / 60)}時間${minutes % 60}分`}</span><span>{card.room.participantCount.status === 'available' ? `登録${card.room.participantCount.value}人` : '人数未取得'}</span></div>
    ...
    <OutlawButton loadingLabel="" fullWidth aria-label="救援先を開く" onClick={() => { openRaidRescue(rescueId); onOpen?.(); }}>{ended ? '結果を見る' : '救援に向かう'}</OutlawButton>
  </section>;
}
