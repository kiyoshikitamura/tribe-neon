...
export function BattleRankingSummary({ rank, rate, onOpen }: { rank?: number | null; rate: number; onOpen: () => void }) {
  return <section className="battle-top-ranking" aria-label="Battleランキング"><div><h2>BATTLE RANKING</h2><small>現在</small><strong className={rank != null && rank > 0 ? undefined : "is-unranked"}>{rank != null && rank > 0 ? `${rank}位` : "順位未取得"}</strong><p>RATE {rate.toLocaleString()}</p></div><OutlawButton variant="secondary" onClick={onOpen}>ランキングを見る</OutlawButton></section>;
}
