...
export default function RankingTab() {
  ...
  const activeTab: RankingCategory = RANKING_TABS.some((tab) => tab.id === rankingActiveTab)
    ...
    : "power";
  const journeyRewardIds = rankingMissionRewardOrigin && beginnerJourney?.facts.pvp ? beginnerRewardIds(beginnerJourney, "pvp") : [];
  const [rewardOpening, setRewardOpening] = useState(false);
  const [journeyCheck, setJourneyCheck] = useState<'loading' | 'ready' | 'error'>('loading');
  const [journeyRetry, setJourneyRetry] = useState(0);
  useEffect(() => {
    if (!rankingMissionRewardOrigin) return;
    let current = true;
    setJourneyCheck('loading');
    void refreshBeginnerJourney().then((ok: boolean) => {
      if (current) setJourneyCheck(ok ? 'ready' : 'error');
    }).catch(() => { if (current) setJourneyCheck('error'); });
    return () => { current = false; };
  }, [rankingMissionRewardOrigin, session?.user?.id, refreshBeginnerJourney, journeyRetry]);
  const journeyPending = rankingMissionRewardOrigin && !beginnerJourney?.facts.pvp;
  const journeyFailed = journeyPending && journeyCheck !== 'loading';
  const openJourneyReward = async () => {
    if (rewardOpening || !journeyRewardIds.length) return;
  ...
  return (
    <HubPage className="ranking-tab-view" title="" hideVisualHeader status={readiness.status} onRetry={readiness.retry}>
      <div className="ranking-single-banner" aria-label="ランキング">
        <img src="/promotion/mypage_banner_ranking.webp" alt="ランキング" />
      </div>
      ...
      <section className="ranking-current" aria-label="あなたの現在地">
        <div className="ranking-current-identity">
          {activeTab === "guild_power" ? <div><small>YOUR GUILD</small><strong>{userGuild?.name ? <GuildIdentity guildId={userGuild.id} name={userGuild.name} size="m" /> : "未所属"}</strong></div> : currentIdentityName ? <UserIdentityRow userId={currentUserId} userName={currentIdentityName} guildName={currentProfile?.guild_name || userGuild?.name} guildId={currentProfile?.guild_id || userGuild?.id} leaderCharacterId={currentProfile?.favorite_character_id || currentUser?.favorite_character_id} onOpen={currentUserId ? () => openPlayer(currentUserId) : undefined} variant="compact" /> : <div className="ranking-current-identity-loading" role="status">ユーザー情報を取得中</div>}
        </div>
        ...
        : loading ? <div className="ranking-skeleton" aria-label="ランキング取得中">{[0, 1, 2].map((key) => <span key={key} />)}</div>
          : activeTab === "guild_power" ? <div className="ranking-list">{displayedRows.length > 0 ? displayedRows.map((row) => {
            ...
            : <div className="ranking-list">{displayedRows.length > 0 ? displayedRows.map((row) => {
              ...
              return <article key={row.user_id} className={`ranking-user-row ${row.user_id === currentUserId ? "is-current" : ""}`}><span className={`ranking-position is-${rank || "out"}`}><RankPresentation rank={rank} /></span><div className="ranking-user-main"><UserIdentityRow userId={row.user_id} userName={profile?.username || row.username || "プレイヤー"} guildName={profile?.guild_name || row.guild_name} guildId={profile?.guild_id || row.guild_id} leaderCharacterId={profile?.favorite_character_id} onOpen={() => openPlayer(row.user_id)} variant="compact" /><RankingDeck characterIds={profile?.main_formation_character_ids} /></div><span className="ranking-metric">{metric}<small>{activeTab === "power" ? "総合力" : activePeriod === "daily" ? "WIN" : "RATE"}</small></span></article>;
