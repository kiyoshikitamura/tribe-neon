      ...
      {!selectionVisible && !selectedPatrol && <section className="quest-v2-overview" aria-label="派遣状況">
        ...
        {activePatrols.map((patrol: any) => {
          ...
          return <button className="quest-v2-dispatch-card" data-state={state} key={patrol.id} onClick={() => openPatrol(patrol.id)}>
            ...
            <span className="quest-v2-dispatch-description"><strong>{character?.jpName || "派遣担当"}</strong><span>{course?.name || "クエスト"} / {difficulty(course?.level_type || "")}</span><b>{label}{state === "WAITING" ? `・残り ${clock(patrol.secondsLeft)}` : ""}</b>{isCharacterHometown(character?.homeTown, course?.town_id) && <em className="quest-v2-bonus">地元一致</em>}</span>
