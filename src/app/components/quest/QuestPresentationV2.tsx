...
export default function QuestPresentationV2() {
  ...
  const confirmPaidShorten = (patrolId: string) => {
    ...
    isOpen: true, title: "ダイア時短", message: "30ダイアを消費して、このクエストの待機時間を完了します。バトルと報酬受取は別途必要です。",
  ...
  return <HubPage className="patrol-container quest-v2-shell" title="クエスト" hideVisualHeader>
    <div ref={contentRef} className="quest-v2-content" style={{ "--quest-state-background": `url(${stateBgImage})` } as React.CSSProperties}>
      ...
      {!selectionVisible && !selectedPatrol && <section className="quest-v2-overview" aria-label="探索状況">
        <header className="quest-v2-page-hero"><img src="/promotion/mypage_banner_quest.webp" alt="クエスト" /><p>街を探索し、報酬を手に入れろ。</p></header><div className="quest-v2-overview-heading"><span>探索枠 {activePatrols.length} / 5</span></div>
      ...
      {activeCourse && selectionStep !== "DESTINATION" && <section className="quest-v2-brief">
        ...
        <OutlawButton variant="primary" fullWidth disabled={activePatrols.length >= 5} onClick={() => setSelectionStep("CHARACTER")}>キャラクターを選ぶ</OutlawButton></>}
        ...
        {(activePatrols.length >= 5 || Number(game.vitality) < Number(activeCourse.cost_vitality)) && <p className="quest-v2-dispatch-reason" role="status">{activePatrols.length >= 5 ? "探索枠がいっぱいです" : "エナジーが不足しています"}</p>}
