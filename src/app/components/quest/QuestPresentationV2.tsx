  ...
  return <HubPage className="patrol-container quest-v2-shell" title="クエスト" hideVisualHeader>
    ...
    <div ref={contentRef} className="quest-v2-content" style={{ "--quest-state-background": `url(${stateBgImage})` } as React.CSSProperties}>
      ...
      {!selectionVisible && !selectedPatrol && <section className="quest-v2-overview" aria-label="探索状況">
        <header className="quest-v2-page-hero"><img src="/promotion/mypage_banner_quest.webp" alt="クエスト" /><p>街を攻略し、次のステージへ。</p></header><div className="quest-v2-overview-heading"><span>探索枠 {occupiedCount} / 5</span><span>クリア {clearedCount} / 21</span></div>
      ...
      {activeCourse && selectionStep !== "DESTINATION" && <section className="quest-v2-brief">
        ...
        {!activeCourse.is_first_cleared && <section className="quest-v2-enemy-preview" aria-label="出現エネミー"><h3 className="quest-v2-section-title">出現エネミー <span>候補</span></h3><p>編成は探索ごとに変わります</p><div>{enemyCandidates.map((master: any) => <article key={master.id}><CharacterPresentation src={master.img?.startsWith("/characters/") ? master.img : `/characters/${String(master.img || "").replace(/^\//, "")}`} alt={master.jpName} variant="thumbnail" rarity={master.rarity} backgroundSrc={getCharacterLocationBackground(master.homeTown)} frameKind="character" metadata={false} /><strong>{master.jpName}</strong></article>)}</div>{enemyCandidates.length === 0 && <p role="status">出現情報を確認中</p>}</section>}
        {activeCourse.is_first_cleared && <p className="quest-v2-hometown-note">クリア済み：探索完了後に報酬を受け取れます。ボス戦はありません。</p>}
        <h3 className="quest-v2-section-title">報酬</h3>
        {activeCourse.normal_reward_timing && <p className="quest-v2-hometown-note">{activeCourse.is_first_cleared ? "周回報酬：探索完了後に受け取れます。" : activeCourse.normal_reward_timing === "BOSS_VICTORY" ? "通常報酬はボス撃破時に獲得できます。" : "通常報酬は探索完了時に獲得できます。"} 初回クリア報酬はボス撃破時のみ獲得できます。</p>}
