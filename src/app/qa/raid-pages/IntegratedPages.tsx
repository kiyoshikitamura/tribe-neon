...
export default function IntegratedPages({ scenario, now, onAction }: { scenario:string; now:number; onAction:(value:string)=>void }) {
  const connection = useMemo(() => {
    ...
    const loadEnemyInfo:RaidEnemyLoader=async variant=>{const enemy=RAID_TOP_ENEMIES.find(entry=>entry.variantId===variant);if(!enemy)throw Error('Unknown canonical enemy');return {variantId:variant,maxHp:1000000,memberCharacterIds:enemy.roster.map(member=>member.id),members:enemy.roster.map(member=>({characterId:member.id,hp:200000,atk:1000,def:1000,equipmentIds:[]})),skillsByCharacterId:fixtureEnemySkills(variant),clearPlan:fixture.plan,rescuePlan:fixture.plan};};
