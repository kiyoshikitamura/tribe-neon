...
type Kind = "normal" | "skill" | "heal" | "finish" | "buff" | "debuff" | "status" | "shield" | "regen" | "counter" | "cleanse";
...
export default function BattlePresentationMock() {
  ...
  const skillName = stateAction ? kind==="cleanse" ? "弱体解除" : statusCatalog[activeStatus].name : kind === "normal" ? "通常攻撃" : kind === "heal" ? "回復スキル" : "ストリートパンチ";
  ...
  useEffect(()=>{
    ...
    const timer=window.setTimeout(()=>{
      ...
      if(phase==="actor"){
        const value=stateAction ? 0 : kind==="heal" ? Math.min(800,2400-allyHp) : Math.min(hp[target],kind==="normal" ? 400 : kind==="finish" ? 2400 : 1200);
        setAmount(value);
        if(stateAction){
        ...
        if(!m)a.playSe(stateAction ? allyTarget ? "BATTLE_BUFF" : "BATTLE_DEBUFF" : kind==="heal" ? "BATTLE_BUFF" : tier===3&&kind!=="normal" ? "BATTLE_CRITICAL" : "BATTLE_DAMAGE");
        setPhase("impact");
  ...
  },[phase,actor,kind,tier,speed,sequence,busy,allyHp,hp,target,stateAction,allyTarget,activeStatus,statuses]);
  ...
  function roster(c: Character,index:number,enemy:boolean){
    ...
    return <div key={`${enemy}-${c.id}`} className={`bm-unit ${enemy?"enemy":"ally"} ${!enemy&&index===0&&busy?"acting":""} ${affected?"affected":""} ${health===0?"defeated":""}`} data-unit={`${enemy?"enemy":"ally"}-${index}`} data-positive={allyTarget} data-has-status={Boolean((statuses[`${enemy?"enemy":"ally"}-${index}`]||[]).length)}>
      ...
      {affected&&!stateAction&&<><b className={`bm-damage ${kind==="heal"?"heal":""}`}>{kind==="heal"?"+":"−"}{amount.toLocaleString()}</b>{phase==="impact"&&kind!=="heal"&&<img className={`bm-impact ${kind==="normal"?"normal":""}`} src={impactAsset} alt=""/>}</>}
  ...
  return <main ref={rootRef} className={`bm-root bm-tier-${actor.rarity} bm-phase-${phase}`} style={{"--bm-rate":speed} as CSSProperties}>
    ...
    <section className="bm-controls" aria-label="モック操作"><div className="bm-selection"><label>発動キャラ<select disabled={busy} value={actor.id} onChange={e=>{setActor(CHARACTERS_MASTER.find(c=>c.id===e.target.value)!);setStatuses({});}}>{CHARACTERS_MASTER.map(c=><option key={c.id} value={c.id}>{c.rarity} {c.jpName}</option>)}</select></label><button onClick={()=>setSpeed(s=>s===1?2:1)}>×{speed}</button><button onClick={()=>{setMuted(m=>!m);void audio.unlockAudio();}}>SE {muted?"OFF":"ON"}</button></div>
      ...
      <div className="bm-buttons"><button disabled={busy} onClick={()=>playStatus("ATK_UP")}>バフ</button><button disabled={busy} onClick={()=>playStatus("DEF_DOWN")}>デバフ</button><button disabled={busy} onClick={()=>playStatus("SHIELD")}>シールド</button><button disabled={busy} onClick={()=>playStatus("REGEN")}>継続回復</button></div>
