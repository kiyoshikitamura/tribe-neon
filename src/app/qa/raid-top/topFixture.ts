...
export const TOP_SCENARIOS = ['approved', 'empty', 'single', 'multiple', 'loading', 'error', 'unavailable', 'ended', 'long-name', 'no-guild', 'broken-image', 'unknown', 'returned'] as const;
...
export function createTopFixture(scenario: TopScenario, now = Date.now(), areaOffset = 0): RaidTopData {
  const enemyAt = (index: number) => RAID_TOP_ENEMIES[((scenario === 'approved' && index < 2 ? 1 - index : index) + areaOffset) % RAID_TOP_ENEMIES.length];
  ...
  const entry = (index: number, rescue: boolean): RaidTopEntry => {
    ...
    return {
      room: { roomId: `qa-top-${rescue ? 'rescue' : 'joined'}-${index}`, difficultyId: ['beginner', 'intermediate', 'advanced', 'expert'][index % 4] as 'beginner', owner: known(player(scenario === 'approved' && rescue ? 2 : index)),
  ...
  return { participating: { status: 'ready', data: Array.from({ length: scenario === 'approved' ? 2 : count }, (_, i) => entry(i, false)) }, rescues: { status: 'ready', data: Array.from({ length: scenario === 'approved' ? 1 : count }, (_, i) => entry(i, true)) }, dailyTargets: { status: 'ready', data: { dateJst: '2026-09-09', targets: [enemyAt(0), enemyAt(1)] } }, canCreate: true };
}
