...
type Metric = { value: number | null; numerator: number | null; denominator: number | null; status: string; reason?: string | null; authority?: string; authority_label?: string | null };
type DayRow = { date: string; new_users: number; tutorial: Metric; guild: Metric & { create?: number | null; join?: number | null }; chat: Metric; retention: Array<Metric & { day: number }> };
...
function Rate({ metric }: { metric?: Metric }) {
  const hasCounts = metric?.numerator != null && metric?.denominator != null && metric.denominator > 0;
...
export default function KpiDailyOverview() {
  ...
  return <main className="kpi-shell daily-shell">
    ...
    {data && <>
      <div className="daily-desktop"><table><thead><tr><th>日付</th><th>新規</th><th>Tutorial</th><th>Guild</th><th>Chat</th>{[1,2,3,4,5].map((day) => <th key={day}>D{day}</th>)}</tr></thead><tbody>{data.rows.map((row) => <tr key={row.date}><td><DayLink row={row}>{row.date}</DayLink></td><td><b>{row.new_users.toLocaleString("ja-JP")}人</b></td><td><Rate metric={row.tutorial} /></td><td><Rate metric={row.guild} /></td><td><Rate metric={row.chat} /></td>{row.retention.map((item) => <td key={item.day}><Rate metric={item} /></td>)}</tr>)}</tbody></table></div>
      <div className="daily-mobile">{data.rows.map((row) => <DayLink row={row} className="daily-card" key={row.date}><header><time>{row.date}</time><span>詳細 →</span></header><div className="daily-primary"><span><small>新規ユーザー</small><b>{row.new_users.toLocaleString("ja-JP")}人</b></span><span><small>Tutorial</small><Rate metric={row.tutorial} /></span><span><small>Guild</small><Rate metric={row.guild} /></span><span><small>Chat</small><Rate metric={row.chat} /></span></div><div className="daily-retention">{row.retention.map((item) => <span key={item.day}><small>D{item.day}</small><Rate metric={item} /></span>)}</div></DayLink>)}</div>
