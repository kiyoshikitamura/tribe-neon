  ...
  return <>
    ...
    {data && <>
      ...
      <div className="daily-desktop daily-period-table" role="region" aria-label={`${label}KPI一覧・横スクロール`} data-period={mode} tabIndex={0}>
        <table><thead><tr><th scope="col">{mode === "daily" ? "日付" : "月"}</th><th scope="col">新規ユーザー</th>{mode === "monthly" && <th scope="col">累計登録ユーザー</th>}<th scope="col">{mode === "daily" ? "DAU" : "MAU"}</th><th scope="col">Tutorial Complete</th><th scope="col">Guild Conversion</th><th scope="col">Guild Chat Activation</th>{mode === "daily" && [1, 2, 3, 4, 5].map((day) => <th scope="col" key={day}>D{day}</th>)}{(mode === "daily" ? ["DPU", "DPUR", "Revenue", "DARPPU", "DARPU"] : ["MPU", "MPUR", "Revenue", "MARPPU", "MARPU", "Active Guild", "Effective Active Guild"]).map(label => <th scope="col" key={label}>{label}</th>)}</tr></thead>
          <tbody>{data.rows.map((row) => <tr key={row.date}>
            ...
            <td><b>{number(row.new_users)}{row.new_users != null && "人"}</b></td>{mode === "monthly" && <td>{number(row.total_registered)}</td>}<td className="daily-active-count"><b>{number(row.active_users)}</b></td><td><Rate metric={row.tutorial} /></td><td><Rate metric={row.guild} /></td><td><Rate metric={row.chat} /></td>
...
export default function KpiDailyOverview() {
  ...
  return <main className="kpi-shell daily-shell">
    ...
    <p className="daily-period-note">{mode === "monthly" ? "MAUは月内の利用者を重複除外。累計登録は月末時点（当月は集計時点）。Guild数は月内に日次条件を満たした重複除外数。月をタップして日次一覧へ。" : "DAUは当該JST暦日の利用者を重複除外。日付をタップして日次詳細へ。"} 横にスクロールして全項目を比較できます。</p>
