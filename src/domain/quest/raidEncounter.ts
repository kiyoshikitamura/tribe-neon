export type EncounterDifficulty = 'beginner' | 'intermediate' | 'advanced' | 'expert';
...
export function parseQuestRaidEncounter(value: unknown): QuestRaidEncounter {
  ...
  if (v.status === 'CREATED' && (typeof v.roomId !== 'string' || typeof v.areaId !== 'string' || typeof v.bossName !== 'string' || !['beginner','intermediate','advanced','expert'].includes(String(v.difficulty)))) throw new Error('強敵の情報を確認できませんでした。');
  return {
    patrolId:v.patrolId, status:v.status as QuestRaidEncounter['status'], roomId:typeof v.roomId==='string'?v.roomId:null,
    areaId:typeof v.areaId==='string'?v.areaId:'', difficulty:['beginner','intermediate','advanced','expert'].includes(String(v.difficulty))?v.difficulty as EncounterDifficulty:null,
