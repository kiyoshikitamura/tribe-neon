-- 受取確定レスポンスに必要投影を同梱。報酬の条件/量/ledgerは既存Authorityを維持。
...
revoke all on function public.get_current_mission_reward_state() from public,anon;
...
-- リーダー変更と地元への拠点変更を同じ更新トランザクションにする。
-- 手動拠点移動/同じリーダーの再保存では移動しない。既存拠点一括上書きなし。
...
revoke all on function public.on_leader_hometown_base() from public,anon,authenticated;
create trigger leader_hometown_base before update of favorite_character_id on public.users
for each row execute function public.on_leader_hometown_base();
