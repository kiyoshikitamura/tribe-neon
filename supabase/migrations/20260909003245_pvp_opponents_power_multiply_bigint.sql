-- Promote the operand BEFORE multiplication; preserve every selection and rating rule.
DO $guard$ BEGIN
IF md5(pg_get_functiondef('public.get_pvp_opponents_page(uuid,integer,integer)'::regprocedure)) <> '0e14d5a126a5efea924a6de341006f97'
AND md5(pg_get_functiondef('public.get_pvp_opponents_page(uuid,integer,integer)'::regprocedure)) <> '636f7e768a3a554ba2b4a7f02f4bf372'
THEN RAISE EXCEPTION 'Unexpected opponent RPC definition; audit before applying'; END IF;
END $guard$;
