-- SAFE_ADDITIVE: 既存Fact・DAU・ゲームRPCを変更せず、日次投影だけを追加する。
...
$engagement$;
revoke all on function public.kpi_daily_engagement_v1(date,date) from public,anon,authenticated,service_role;
comment on function public.kpi_daily_engagement_v1(date,date) is
  '日次Social/Raid KPI。JST日末所属・Canonical DAUを使用。SQL分析/既存定期集計専用。';
