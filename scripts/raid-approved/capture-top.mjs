...
const url = process.env.RAID_TOP_QA_URL || 'http://127.0.0.1:3023/qa/raid-top';
const out = 'docs/development/evidence/raid-approved-top';
...
for(const scenario of ['single','multiple','empty','loading','error','unavailable','ended','long-name','no-guild','broken-image','unknown','returned']) {
 ...
 if(scenario!=='loading'){const hit=await page.getByRole('button',{name:/開催中を探す/}).evaluate(el=>{const r=el.getBoundingClientRect();return el.contains(document.elementFromPoint(r.x+r.width/2,r.y+r.height/2));});assert.ok(hit,'browse CTA footer overlap: '+scenario);}
...
await page.getByLabel('Mockシナリオ',{exact:true}).selectOption('empty');await page.getByTestId('raid-top').waitFor();await page.getByRole('button',{name:'この敵に挑む',exact:true}).first().click();await page.getByRole('dialog').waitFor();assert.match(await page.getByTestId('last-navigation').textContent(),/^choose:RAID_/);await page.getByRole('button',{name:'戻る',exact:true}).click();
...
await page.setViewportSize({width:390,height:600});await page.screenshot({path:`${out}/top-returned-low.png`,fullPage:false});
