...
const root='docs/development/raid-production-release',json=p=>JSON.parse(fs.readFileSync(p,'utf8'));
...
const d=await api('/v13/deployments/dpl_D6u4GxSvENg9HxG7V9G1S2tXwuRw');
...
const before=json(root+'/deployment-before.json').aliases;const changes=aliases.filter(x=>before.find(y=>y.alias===x.alias)?.deploymentId!==x.deploymentId);const removed=before.filter(x=>!aliases.some(y=>y.alias===x.alias));
...
assert.equal(state.source,'c844534f5ce160fb3e713a58aed8377190aaa209');assert.equal(removed.length,0);assert.ok(changes.every(x=>['www.tribe-neon.com','tribe-neon.com'].includes(x.alias)&&x.deploymentId===d.id),'Non-game alias changed');
...
let js='';for(const s of scripts){const r=await fetch(new URL(s,'https://'+d.url));assert.equal(r.status,200);js+='\n'+await r.text();}
