...
const approved=JSON.parse(fs.readFileSync('docs/development/quest_identity_review_candidate_20260914.json','utf8'));
...
for(const quest of q.quests){
 ...
 assert.deepEqual(pool.items,approved.rows.find(x=>x.poolId===quest.rewardPoolId).rolls,'Approved 21-pool contract mismatch');
...
console.log('PASS: 21 separate pools, approved CASH, approved region probabilities, enemy rarity feasibility, truthful area reward summaries');
