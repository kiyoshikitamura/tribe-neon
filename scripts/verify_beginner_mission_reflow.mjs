...
function harness() {
  ...
  const projection=()=>({owner,cash:10,diamonds:0,level:1,xp:0,items:[],missions:[{mission_id:'target',status:server,current_progress:1}]});
  let projectionReads=0, bootstrapCalls=0;
  const supabase={rpc:async(name)=>{
    if(name==='get_current_mission_reward_state'){projectionReads++;return {data:projection()};}
    calls++;if(mode==='fail')throw Error('network');server='CLAIMED';if(mode==='lost')throw Error('response lost');
    return {data:{rewards:mode==='noop'?[]:[{item_id:'CASH',quantity:10}],mission_state:projection()}};
  }};
  const exports={};
  ...
  const render=()=>{index=0;return exports.useInventory({user:{id:owner}},0,()=>{},0,()=>{},0,()=>{},()=>{},async()=>{bootstrapCalls++;},value=>{dialog=value;});};
  let hook=render();hook.setMissions([{id:'target',status:'CLEAR',category:'DAILY'}]);hook=render();
  return {get hook(){return hook;}, get dialog(){return dialog;},get returned(){return returned;},get calls(){return calls;}, get projectionReads(){return projectionReads;}, get bootstrapCalls(){return bootstrapCalls;}, callback:()=>returned++,mode:v=>mode=v,rerender:()=>hook=render(),swap:()=>{owner='other';hook=render();}};
}
for (const bulk of [false,true]) {
  ...
  const h=harness();await receive(h,h.callback);assert.equal(h.returned,0,'no navigation during receipt');assert.equal(h.projectionReads,0,'successful grant needs no follow-up state read');assert.equal(h.bootstrapCalls,0,'receipt must not wait for whole-game bootstrap');h.dialog.onConfirm();assert.equal(h.returned,1,'confirmed receipt reflows');h.rerender();await receive(h,h.callback);assert.equal(h.calls,1,'retry of claimed row does not issue another grant');
