...
assert.ok(replaySource.includes('previousReplayEvent?.type === "ACTION" && exclusiveSkill ? EXCLUSIVE_SKILL_PREFIX_MS : 0' ),'delay the authoritative outcome, not only CSS');
const streetSource = fs.readFileSync('src/app/components/battle/StreetBattleViewer.tsx','utf8');
assert.ok(streetSource.includes('key={action?.unit.replayStartCursor ?? props.skillCutIn?.actionKey}'),'every activation restarts full dialogue');
...
const linkedCue = { ...cue, actorId: 'player-ageha', skillId: 'SKILL_055', actionKey: 42 };
const linkedActor = { id: 'player-ageha', characterId: 'char_ageha_01', rarity: 'SSR', skills: [{id:'SKILL_055', name:'ネオン・アクセル'}] };
assert.ok(resolver(linkedCue, linkedActor).dialogue);
assert.equal(resolver(linkedCue, {...linkedActor,id:'enemy-ageha'}),null,'same character/name cannot substitute for event actor');
assert.equal(resolver({...linkedCue,skillId:'SKILL_001'},linkedActor).dialogue,undefined,'explicit unknown skill cannot fall back to name');
assert.ok(replaySource.includes('actorId, skillId, actionKey: authoritativeEventIndex'));
assert.ok(streetSource.includes('props.skillCutIn ? props.skillCutIn.actorId :'));
console.log('PASS: atomic actor/skill cue, duplicate-name rejection, explicit skill authority, cue actor priority, fallback prefix wait');
