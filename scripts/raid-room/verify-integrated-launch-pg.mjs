...
try{
 ...
 await db.exec('begin');
 await db.exec(read('docs/development/raid-launch-integration/bundle/06-launch-balance.sql'));
 await db.exec(read('docs/development/raid-launch-integration/bundle/07-launch-postflight.sql'));
 await db.exec('rollback');
 assert.deepEqual(await functions(),beforeFunctions);
 assert.equal(await value("select to_regclass('public.raid_room_combat_profiles') value"),null);
 out.checks.push('Integrated delta rollback restores function definitions and removes added tables');
 await db.exec('begin');
 await db.exec(read('docs/development/raid-launch-integration/bundle/06-launch-balance.sql'));
 await db.exec(read('docs/development/raid-launch-integration/bundle/07-launch-postflight.sql'));
...
}catch(e){out.status='FAIL';out.error={message:e.message,code:e.code,stack:e.stack};process.exitCode=1;}finally{await db.close();fs.mkdirSync(dir,{recursive:true});fs.writeFileSync('docs/development/raid-launch-integration/validation-pg.json',JSON.stringify(out,null,2)+'\n');console.log(JSON.stringify({status:out.status,checks:out.checks,snapshots:out.snapshots.length,error:out.error}));}
