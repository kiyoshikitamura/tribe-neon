...
function fixture({points=0,free=false,tickets=2,fail=false}={}) {
 ...
 const dialog=()=>find(render(),n=>['レイドチケットで回復しますか？','チケットを使って挑戦しますか？'].includes(n.props?.title));
...
const checks=[];
let f=fixture();await f.enter();assert.ok(f.dialog());assert.equal(f.calls,0);f.dialog().props.actions[0].onClick();assert.equal(f.dialog(),null);assert.equal(f.calls,0);assert.equal(f.prepared,0);checks.push('zero RP opens; close consumes nothing and does not prepare');
...
f=fixture();await f.enter();const release=f.hold();const action=f.dialog().props.actions[1];assert.equal(action.label,'チケット1枚を使って挑戦');action.onClick();action.onClick();assert.equal(f.calls,1);assert.equal(f.dialog().props.actions[1].disabled,true);release();await new Promise(r=>setImmediate(r));assert.equal(f.context.raidPoints,1);assert.equal(f.dialog(),null);assert.equal(f.prepared,1);assert.equal(f.calls,1);checks.push('double click consumes one; automatically prepares the selected raid');
