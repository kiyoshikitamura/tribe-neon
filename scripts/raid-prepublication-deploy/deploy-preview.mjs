...
const root=path.resolve(import.meta.dirname,'../..'), out=path.join(root,'outputs/raid-prepublication-deploy');
const sourceSha='8c0fa6b2131faaed7b63e172ac12424d3fe8f6be';
...
if(path.resolve(stage)!==path.join(out,'source-8c0fa6b'))throw Error('Wrong staging path');
...
const safe={sourceSha,project,target:'preview (target omitted)',autoAssignCustomDomains:false,alias:[],gitSourceAbsent:true,gitRefAbsent:true,fileCount:files.length,bytes:files.reduce((n,f)=>n+f.size,0),envKeys:Object.keys(env)};
...
if(process.argv.includes('--auth-check')){console.log('PASS read-only credential loader GET: expected project/team and all captured alias bindings match');process.exit(0);}
