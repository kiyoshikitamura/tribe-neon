...
## 盤面維持・倍速認識時間の調整
- SSRは別背景を撤去。既存の盤面へ透過立ち絵（不透明度88%）を重ね、立ち絵のドロップシャドウと文字周辺の半透明影のみ使用。
- N/R/SRは顔アイコン＋名前＋スキル名＋セリフを表示。発動元の枠強調も維持。
- 倍速でも発動表示N/R/SRは800ms、SSRは1200ms以上。Impactは450ms以上、settleは250ms以上で数値表示は計700ms以上。次の待機・進行中もsettleまで数値を残す。
- CSSエフェクト時間とJSのImpact時間を合わせ、一律速度除算を廃止。reduced-motionでも認識時間を維持。再生途中の倍率切替によるタイマー再起動を避け、再生中は倍率ボタンを無効化。
- 本番・ガチャ・バトル計算の変更なし。
