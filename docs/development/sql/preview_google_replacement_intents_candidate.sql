-- Preview専用候補。未適用。Production適用禁止。
...
declare
  ...
  v_safe_tables text[] := array['users','user_characters','user_skills','user_equipments','user_items',
    'user_account_auth_methods','user_decks','user_deck_characters','presents'];
begin
  ...
  for v_constraint in
    ...
    where fk.contype='f' and fk.confrelid='public.users'::regclass
      and not (n.nspname='public' and c.relname=any(v_safe_tables))
  loop
