...
親追加承認: 限定Roomブロック分割でOOMが残る場合、startCardBattleInternal全体をsrc/hooks/battle/専用モジュールへ機械的に移譲可。本文の計算・分岐順・await・状態遷移を変えずclosureを明示contextへ置換する。本文比較・既存戦闘検証・通常lint・型検証を確認する。

親レビュー: 開始本文107文/53,372文字をASTで比較しcontext代入を除き一致。全体通常lint0errors・型・Mockbuild・実hook17PASS。除外/ルール緩和なし。CI再実行は別途待ち。
