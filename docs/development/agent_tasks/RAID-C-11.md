# RAID-C-11 — サーバーからの戦闘復帰・未開始要求取消
TASK ID: RAID-C-11
OWNER: raid_c_phase11
PRIORITY: P1
STATUS: VALIDATED
SCOPE: tests/db SQL258実fixture/runnerとtests/raid-room domain/実hookのみ専有、phase11_validation文書。開始→取消がstarted、取消→遅い開始が拒否、本人限定、FINALIZED前ack不可、ack冪等、一覧制限/ack除外、local消失復帰/未確認一覧空では自動出撃なし、cancel結果に応じるUI動作を検証。
