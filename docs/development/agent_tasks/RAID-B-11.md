# RAID-B-11 — サーバーからの戦闘復帰・未開始要求取消
TASK ID: RAID-B-11
OWNER: raid_b_phase11
PRIORITY: P1
STATUS: VALIDATED
SCOPE: src/domain/raidRoomBattlePending.ts/Attempt.ts、新規復帰補助module、useBattle.ts、GameContext.tsxと必要UIのみ（battleTypes.tsの任意GlobalInteractionBlocker引数を含む）。local記録なしでも本人未確認一覧を見てserver payload/receiptを復帰（自動新規出撃しない）。確定＋互換保存到達後ackしてlocalclear。valid local pendingのreceipt未取得/開始拒否時に共通ConfirmDialogで未開始要求取消→cancel RPCのstartedなら同receipt復帰、cancelledのみlocalclear。既存記録破損は保存済みserver recordの復帰のみ許可、未知requestの安全破棄を捏造しない。旧アカウント応答guard/同時復帰guard維持。phase11_ui文書。
