do $guard$ begin
 if md5((select jsonb_agg(jsonb_build_object('name',p.proname,'args',pg_get_function_identity_arguments(p.oid),'md5',md5(pg_get_functiondef(p.oid)),'owner',pg_get_userbyid(p.proowner),'acl',p.proacl) order by p.proname,pg_get_function_identity_arguments(p.oid)) from pg_proc p where p.pronamespace='public'::regnamespace and p.prokind='f')::text) is distinct from 'f4015bd5da10bae06204565726765eac' then
  raise exception '工程2 baseline drift: public function body/signature/owner/ACL。再取得・再照合が必要';
 end if;
end $guard$;
do $guard$ begin
 if md5((select jsonb_agg(t order by version) from (select version,name from supabase_migrations.schema_migrations)t)::text) is distinct from '8afe7af54d5c7870a687c74152ec8dfa' then
