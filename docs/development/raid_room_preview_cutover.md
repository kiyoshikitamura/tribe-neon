...
2026-09-08 / RAID-A-17 / 文書作成完了・親レビュー前。
照合基準: `45dcaba09ac0872c976b5fdcbf773beb096813f2`（PR #27、第16工程）。この文書は実行手順の準備であり、実DB適用・配信・有効化・実機確認の完了記録ではない。

## 現時点の結論

**DB → Edge → UI の順で独立Previewへ接続する。旧Raidの新規生成・開始を止める実装が残っているため、現行設定だけで新Roomへ全面切替できない。** 数値は変更可能な設定として投入し、HP導出研究を準備の開始条件にしない。

|経路|固定SHAで確認した動作|切替時の扱い|
|---|---|---|
|旧一覧 `get_active_raids()`|SQL254で `rotate_daily_raids()` を呼び、日次生成・5分後respawnを進める|純粋な読取ではない。DB監査の読取RPCとして呼ばない|
|旧生成 `rotate_daily_raids()` / `respawn_cleared_raid_slot()`|Room台帳に載るInstanceだけ除外。旧生成停止設定は参照しない|Cron停止だけでは不十分。入口を含めた停止guardが実装残件|
|旧開始 `start_raid_battle()`|旧InstanceのACTIVE・期限・Lv/RP等を判定。Room専用設定は参照しない|新規開始停止guardが実装残件。確定RPCと一緒に停止しない|
|新UIの公開設定|`NEXT_PUBLIC_RAID_ROOM_UI_ENABLED === "true"` でRoom一覧を追加する|旧画面は同時に残る。設定変更だけを旧導線撤去と扱わない|
|起動時の旧一覧取得|`GameContext.tsx` が `get_active_raids` を呼ぶ|Room表示後も旧生成が動き得るため、切替対象に含める|
...
1. **DB差分を確認し、必要な00250〜00262を昇順適用する。** 00254の旧経路分離、00256の確定分岐、00257/258の復帰、00259/260の救援、00261の順位停止、00262の討伐報酬を欠落させない。既適用migrationを無条件で再実行しない。初期運用設定はfalseのまま確認する。
...
5. **テスト用設定を投入し、その後で独立Previewの新規操作を有効化する。** 下表の設定は別物。救援・討伐報酬の値/品目を空のまま「報酬まで通るPreview」と宣言しない。SQL260/262の発行済み台帳を数値調整のために削除しない。
6. **旧生成・開始停止guardと旧UI置換を実装・検証して全面切替を確認する。** 未実装の現段階では新旧併存の限定テストまで。旧開始済みReplayを残した状態で、旧新規開始は拒否・旧確定は成功・新Roomは通常動作することを確認する。
...
|Room画面|`NEXT_PUBLIC_RAID_ROOM_UI_ENABLED`（RaidTab）|表示の追加。サーバー停止や旧画面の置換ではない|
...
固定SHAの読取照合: SQL251/253/254/255/256/257/259/260/261/262、`supabase/functions/resolve-battle/index.ts`、`src/app/components/RaidTab.tsx`、`src/app/context/GameContext.tsx`、`docs/development/raid_room_cutover_remaining.md`。

残件は、旧生成/旧新規開始の停止guard・旧UI置換、実機用設定、独立Preview DB/Edge/UI接続、実Cron/実多接続/実機受入。第17工程で実DB・Deployを実施していない。文書の親レビュー完了は、実機確認可能Preview到達を意味しない。
