with expected(name,args,hash) as (values ('accept_friend_request','p_request_id uuid','0dbb460d66ec4aafa006c203bf538e4a'),
...
('validate_official_battle_result','p_result jsonb','83982e617acdc623d4b80377f013d513')) select count(*) as protected_count,count(*) filter(where p.oid is null or md5(pg_get_functiondef(p.oid)) is distinct from e.hash) as mismatches from expected e left join pg_proc p on p.pronamespace='public'::regnamespace and p.proname=e.name and pg_get_function_identity_arguments(p.oid)=e.args;
