begin read only; set local statement_timeout='60s';
...
select jsonb_build_object('at',clock_timestamp(),'backend_pid',pg_backend_pid(),'ledger',(select jsonb_agg(t) from deployment_audit_raid_v1.applied_changes t),'room_creation',(select enabled from public.raid_room_creation_settings),'room_battle',(select enabled from public.raid_room_battle_settings),'room_rescue',(select enabled from public.raid_room_rescue_settings),'legacy',(select enabled from public.raid_legacy_settings),'room_count',(select count(*) from public.raid_rooms),'transaction_active',exists(select 1 from pg_stat_activity where backend_xid::text='47165'),'audit_rls',(select relrowsecurity from pg_class where oid='deployment_audit_raid_v1.applied_changes'::regclass)) as audit; rollback;
