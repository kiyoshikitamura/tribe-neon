select set_config('raid.operator','Codex / user-authorized Preview execution',false);
select set_config('raid.approval','User approval 2026-09-08: rollback then persist only if all checks match',false);
select set_config('raid.execution_id','731ff89a-5832-4c6b-9267-4fd1317fa902',false);
-- UTF-8, LF. Default ROLLBACK. Set raid.operator / raid.approval / raid.execution_id in this session first.
begin;
...
begin
 ...
 if to_regnamespace('deployment_audit_raid_v1') is not null then
   if to_regclass('deployment_audit_raid_v1.applied_changes') is not null then
     ...
     if found then
       if old_hash='adc3bccfc6345f5fd36dfcdd1d42bbba01531882437f61f50036365ed1d77ffb' then raise exception 'ALREADY_APPLIED';
...
do $execute$
declare payload text := $raid_payload$-- 00232全再適用は禁止。現PreviewのSPD/LUK補正を残し表示metadataだけ追加。
...
$raid_payload$;
begin
 if encode(sha256(convert_to(payload,'UTF8')),'hex') <> 'adc3bccfc6345f5fd36dfcdd1d42bbba01531882437f61f50036365ed1d77ffb' then raise exception 'PAYLOAD_HASH_MISMATCH'; end if;
 execute payload;
end $execute$;
...
values ('sufvuqdnqohpfzkwxohq','raid-room-preview-375a0ad-delta-v1','adc3bccfc6345f5fd36dfcdd1d42bbba01531882437f61f50036365ed1d77ffb',
...
'e693e496d2e7c4ba1f7083615564ae31c04042383948853041e7de62b00ae3f9','{"status":"PASS","checks":"04-postflight.sql","postflight_sha256":"f847a7829caa99cddd6c8798fc3045d699d514e8e53cbab0796310164303ffae"}');
