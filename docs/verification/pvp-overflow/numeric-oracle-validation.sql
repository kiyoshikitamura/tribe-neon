...
CREATE TEMP TABLE oracle_results(x integer);
...
;
CREATE OR REPLACE FUNCTION pg_temp.numeric_oracle(p_user_id uuid, p_my_points integer, p_offset integer DEFAULT 0)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'public'
...
begin
  ...
  with daily_scores as (
    ...
    select player.id,player.username,coalesce(rank.rank_points,1000) rating,
      ...
      case
        when abs(coalesce(rank.rank_points,1000)-coalesce(p_my_points,1000))<=300
          and (coalesce(v_my_power,0)<=0 or coalesce(power.total_power,0)::numeric*10000 between v_my_power*7000 and v_my_power*14000) then 1
        when abs(coalesce(rank.rank_points,1000)-coalesce(p_my_points,1000))<=500
          and (coalesce(v_my_power,0)<=0 or coalesce(power.total_power,0)::numeric*10000 between v_my_power*5000 and v_my_power*18000) then 2
        else 3
      end match_tier
    from public.users player
    join public.user_main_formations formation on formation.user_id=player.id
...
;
DO $audit$ DECLARE u record; o integer; a jsonb; b jsonb; n integer:=0;
BEGIN FOR u IN SELECT p.id,coalesce(r.rank_points,1000) points FROM public.users p LEFT JOIN public.pvp_ranks r ON r.user_id=p.id LOOP
 ...
 a:=public.get_pvp_opponents_page(u.id,u.points,o); b:=pg_temp.numeric_oracle(u.id,u.points,o);
 IF a<>b THEN RAISE EXCEPTION 'numeric reference mismatch'; END IF; n:=n+1;
 END LOOP; END LOOP; INSERT INTO oracle_results VALUES(n); END $audit$;
SELECT x AS numeric_reference_exact_matches FROM oracle_results; ROLLBACK;
