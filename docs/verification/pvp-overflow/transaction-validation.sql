...
CREATE TEMP TABLE overflow_baseline(user_id uuid, points integer, page_offset integer, result jsonb, error_code text) ON COMMIT DROP;
DO $audit$
DECLARE u record; o integer; v jsonb;
BEGIN
 FOR u IN SELECT player.id,coalesce(r.rank_points,1000) points FROM public.users player LEFT JOIN public.pvp_ranks r ON r.user_id=player.id LOOP
 PERFORM set_config('request.jwt.claims',jsonb_build_object('sub',u.id,'role','authenticated')::text,true);
 FOREACH o IN ARRAY ARRAY[0,5,10000] LOOP
 BEGIN
 v:=public.get_pvp_opponents_page(u.id,u.points,o);
 INSERT INTO overflow_baseline VALUES(u.id,u.points,o,v,null);
 EXCEPTION WHEN OTHERS THEN
 INSERT INTO overflow_baseline VALUES(u.id,u.points,o,null,SQLSTATE);
 END;
 END LOOP;
 END LOOP;
END $audit$;
...
;
DO $audit$
DECLARE b record; v jsonb; passed integer:=0; recovered integer:=0;
BEGIN
 FOR b IN SELECT * FROM overflow_baseline LOOP
 PERFORM set_config('request.jwt.claims',jsonb_build_object('sub',b.user_id,'role','authenticated')::text,true);
 v:=public.get_pvp_opponents_page(b.user_id,b.points,b.page_offset);
 IF b.error_code IS NULL AND v<>b.result THEN RAISE EXCEPTION 'baseline mismatch'; END IF;
 IF b.error_code IS NOT NULL AND b.error_code<>'22003' THEN RAISE EXCEPTION 'unexpected baseline error %',b.error_code; END IF;
 IF jsonb_array_length(v->'items')>5 THEN RAISE EXCEPTION 'page size changed'; END IF;
 passed:=passed+1;
 IF b.error_code='22003' THEN recovered:=recovered+1; END IF;
 END LOOP;
 PERFORM set_config('audit.summary',jsonb_build_object('tested',passed,'recovered',recovered,'unchanged',passed-recovered)::text,true);
END $audit$;
SELECT current_setting('audit.summary')::jsonb AS validation;
ROLLBACK;
